...
def apparentMagFromAbsMag(absMag, theDistance):
    ...
    apparentMagFromAbsMag=5*math.log10(theDistance/10.0)+absMag
    return apparentMagFromAbsMag


def solarTemp(SL, SR):
    #returns star temp in K from solar luminocty and solar mass
    solarTemp =((SL /(SR ** 2.0)) ** 0.25) * 5778.0
    return solarTemp


def starCircumference(radius_km):
    starCircumference = 2.0 * math.pi * radius_km
    return starCircumference


def starSurfaceArea(radius_km):
    starSurfaceArea = 4.0 * math.pi *(radius_km ** 2.0)
    return starSurfaceArea


def starVolume(radius_km):
    #return in m**3
    radius_m = radius_km * 1000.0
    starVolume =(4.0 / 3.0) * math.pi *(radius_m **3.0)
    return starVolume
...
def stdGravitationalParameter(mass_kg):
    ...
    return stdGravitationalParameter
